...
export default function ActivitiesPage({ lang, navigateTo, setDonationForm }: ActivitiesPageProps) {
    ...
    <section className="py-16 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-950 mt-1 mb-4">Focused Projects & Welfare Schemes</h1>
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-12">
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-3">Empowering Young Scholastic Dreamers</h2>
              ...
              <div className="grid sm:grid-cols-3 gap-4 mb-4">
                {[
                  { title: "Merit Scholarships", desc: "Focusing on technical and engineering streams." },
                  { title: "Scholastic Toolkits", desc: "Free note-books, geometry boxes, and bags." },
                  { title: "Target Mentoring", desc: "Guidance seminars for final board studies." },
                ].map((item, i) => (
              ...
              <button
                onClick={() => { setDonationForm((prev: any) => ({ ...prev, amount: "1000", message: "Supporting Educational Scholarships" })); navigateTo("donate"); }}
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-12">
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-8 order-2 lg:order-1">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-3">Pristine Medical Diagnostic Camps & Medicine Banks</h2>
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed mb-4">
              ...
              <div className="grid sm:grid-cols-3 gap-4 mb-4">
                {[
                  { title: "Subsidized Diagnosis", desc: "Assisting high-cost lab pathology bills." },
                  { title: "Medicine Subsidies", desc: "Complimentary distribution of critical items." },
                  { title: "Cataract Screenings", desc: "Free optical assemblies and standard glasses." },
                ].map((item, i) => (
                  <div key={i} className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <span className="block font-bold text-slate-900 text-sm">{item.title}</span>
                    <span className="block text-slate-500 text-xs mt-0.5">{item.desc}</span>
              ...
              <button
                onClick={() => { setDonationForm((prev: any) => ({ ...prev, amount: "5000", message: "Supporting Medical Camps" })); navigateTo("donate"); }}
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-12">
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-3">Ensuring Basic Resource & Food Security</h2>
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed mb-4">
              ...
              <div className="grid sm:grid-cols-3 gap-4 mb-4">
                {[
                  { title: "Nutritious Food Kits", desc: "Grains, oil, lentils support packs." },
                  { title: "Monsoon Security Aid", desc: "Shelter components and immediate assets." },
                  { title: "Clean Water Reach", desc: "Setting temporary drinking water posts." },
                ].map((item, i) => (
                  <div key={i} className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <span className="block font-bold text-slate-900 text-sm">{item.title}</span>
                    <span className="block text-slate-500 text-xs mt-0.5">{item.desc}</span>
              ...
              <button
                onClick={() => { setDonationForm((prev: any) => ({ ...prev, amount: "2500", message: "Supporting Community Relief" })); navigateTo("donate"); }}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[
            { img: IMAGES.womenEmpower, tag: "Empowerment", tagColor: "text-rose-600", title: "Vocational Stitching Skill Center", desc: "Driving sewing technology sessions for women and daughters in rural blocks to foster financial confidence and independent business structures." },
            { img: IMAGES.yogaCamp, tag: "Wellness & Mind", tagColor: "text-sky-600", title: "Yoga & Pranayama Wellness Retreats", desc: "Providing comprehensive state wellness guidance sessions, focusing on correct mental alignment, stress release, and traditional wellness guidelines." },
          ].map((item, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              <div className="relative aspect-video w-full rounded-xl bg-slate-100 overflow-hidden mb-4">
                <Image src={item.img} fill alt={item.title} className="object-cover" referrerPolicy="no-referrer" />
              </div>
              <span className={`${item.tagColor} font-bold text-xs uppercase tracking-widest block mb-1`}>{item.tag}</span>
